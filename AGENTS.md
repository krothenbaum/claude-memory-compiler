...
The singleton live worker restores end-of-day compilation after a successful daily append. When its queue drain becomes idle at or after 16:00 local time, it fingerprints content after today's last `@compiled-through` marker. The first immediate SQLite transaction atomically provisions an owner and exactly one takeover watchdog. The scheduler starts the watchdog first with three bounded local spawn attempts, then starts the owner. Total watchdog spawn failure rolls back only the matching unstarted pair; a concurrent pending fingerprint remains in an expired `spawn_failed` reservation for a later retry. Owner spawn failure immediately expires its lease while preserving the watchdog. Every later successful idle drain replaces the pending fingerprint with its latest observation and installs a watchdog only when none is live. During `queue_wait`, `retry_wait`, or `read_wait`, a later drain atomically registers at most one contender even while the watchdog lease is live. The contender polls that predecessor lease and can take over after watchdog death without another post-expiry drain.

The detached owner rechecks the fingerprint, reservation, and absence of pending, failed, or leased jobs. It holds an immediate queue transaction across the `compile.py` launch. Queue work that commits before launch or appears during promotion expires the owner lease into `queue_wait` while preserving the fingerprint and watchdog. The watchdog heartbeats through active queue work and resumes the generation when those jobs become terminal, even if they die and no later drain succeeds. Daily reads carry an explicit `unreadable`, `covered`, or `uncompiled` state; unreadable never means covered and never deletes a request. After a child exits, zero succeeds only when the post-state is readable and covered or its marker tuple is a strict prefix extension of the pre-launch tuple. Unreadable state, a still-uncompiled marker replacement or truncation, zero without progress, and other unchanged failures persist an error class and retry after bounded 5-second exponential backoff, capped at three attempts. Changed content with unchanged markers starts a fresh generation without inheriting the old attempt count.

Watchdog succession is a two-phase external-process protocol, not an atomic claim about `Popen` and SQLite. Before takeover commit, the predecessor spawns a successor with both tokens. If commit succeeds, the successor observes its registered watcher identity. If commit rolls back or the predecessor dies between spawn and commit, the successor waits for the predecessor watcher or contender lease to expire and atomically registers itself. If the predecessor remains live, its heartbeat prevents displacement. SQLite errors make the current observer retry with bounded polling while any already spawned successor remains eligible. Each successful takeover repeats the protocol before running the coordinator, preserving recovery across repeated owner crashes. Exit code 75 remains a deferred overlap signal: the owner retains and renews the request, then polls `scripts/memory-auto-compile.lock` and the marker until the orphan child covers the content or the lock clears for retry.

The scheduler requires zero interactive Claude Code and Codex CLI sessions. POSIX uses bounded argv-list `ps`; Windows uses a five-second, non-interactive argv-list PowerShell CIM query filtered to `claude.exe` and `codex.exe`. It excludes a Claude Agent SDK provider child only when its executable or `argv[0]` is an exact bundled `_bundled/claude` path. It excludes a Codex process only when its argv matches the memory provider's complete fixed `exec` signature, including its safety flags, sandbox, absolute workspace/output relationship, memory-specific output filename, optional absolute schema, and final stdin marker. Partial or prompt-mimicked signatures remain interactive. Process inspection fails closed when unavailable, ambiguous, or unable to read a matching process. A successful empty Windows query means zero sessions. A failed extraction, a drain with no successful daily write, pending or retrying work, an active interactive session, an unreadable daily log, or fully compiled content does not trigger compilation. The coordinator and compile child receive `AI_MEMORY_INTERNAL_JOB=1` to prevent recursive capture.
