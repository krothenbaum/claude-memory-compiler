...
The singleton live worker restores end-of-day compilation after a successful daily append. When its queue drain becomes idle at or after 16:00 local time, it fingerprints content after today's last `@compiled-through` marker and creates a leased SQLite reservation. The detached `auto-compile.py` coordinator rechecks the fingerprint, reservation, and absence of pending, failed, or leased jobs. It holds an immediate queue transaction across the `compile.py` launch, so an enqueue that commits before launch cancels compilation and one that commits afterward belongs to a later drain. The reservation prevents concurrent schedulers from launching the same content, renews while compilation runs, releases after success or failure, and expires after a coordinator crash. An automatic `compile.py` child holds `scripts/memory-auto-compile.lock` for its lifetime, so an orphaned child still rejects an overlapping automatic launch after its coordinator's lease expires.

The scheduler requires zero interactive Claude Code and Codex CLI sessions. It excludes the Claude Agent SDK's bundled provider child and only the memory provider's unambiguous Codex `exec` command shape from the interactive count. It fails closed when process inspection is unavailable or ambiguous. A failed extraction, a drain with no successful daily write, pending or retrying work, an active interactive session, an unreadable daily log, or fully compiled content does not trigger compilation. The coordinator and compile child receive `AI_MEMORY_INTERNAL_JOB=1` to prevent recursive capture.
