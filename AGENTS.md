...
The singleton live worker restores end-of-day compilation after a successful daily append. When its queue drain becomes idle at or after 16:00 local time, it fingerprints content after today's last `@compiled-through` marker and creates a leased SQLite reservation. The detached `auto-compile.py` coordinator rechecks the fingerprint, reservation, and absence of pending, failed, or leased jobs. It holds an immediate queue transaction across the `compile.py` launch, so an enqueue that commits before launch cancels compilation and one that commits afterward belongs to a later drain. A later idle drain replaces the reservation's pending fingerprint with its latest content generation instead of spawning another coordinator. When the compile child exits, the coordinator reads the post-compile fingerprint inside an immediate transaction. It releases if the marker covers all observed content, cancels if queue work arrived, or promotes the current fingerprint and serially runs one coalesced follow-up generation. An unchanged failed generation releases instead of looping; changed content remains retryable. The lease renews while compilation runs and an expired coordinator reservation can be replaced with the latest generation after a crash. An automatic `compile.py` child holds `scripts/memory-auto-compile.lock` for its lifetime, so an orphaned child still rejects an overlapping automatic launch after its coordinator's lease expires.
