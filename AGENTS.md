...
The singleton live worker restores end-of-day compilation after a successful daily append. When its queue drain becomes idle at or after 16:00 local time, it checks today's daily log for content after the last `@compiled-through` marker and starts one detached `compile.py` process. The scheduler requires zero interactive Claude Code and Codex CLI sessions. It excludes the Claude Agent SDK's bundled provider child and Codex `exec` provider children from the interactive count. It fails closed when process inspection is unavailable. A failed extraction, a drain with no successful daily write, pending or retrying work, an active interactive session, an unreadable daily log, or fully compiled content does not trigger compilation. The compile child receives `AI_MEMORY_INTERNAL_JOB=1` to prevent recursive capture.
