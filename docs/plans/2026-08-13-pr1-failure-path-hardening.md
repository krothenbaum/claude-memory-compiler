...
Create or reuse a no-follow, owner-only runtime directory under `AI_MEMORY_HOME/scripts/`. Retain a descriptor for every slice write and validate that the preview path still names that descriptor before and after preview. Use pinned directory-relative creation when available. Without secure `dir_fd`, safely create a fresh root/scripts/runtime one component at a time with no-link/reparse, owner, permission, and pre/post identity checks; fail closed on unsafe ancestry, swaps, or permission-establishment failure. On Windows, traverse root/scripts with pinned no-reparse handles and permit stable current-owner ancestry to retain normal inherited SYSTEM and Administrators ACEs. Establish and verify the runtime's protected current-owner-only DACL through its retained handle. For a slice whose retained CRT descriptor lacks `WRITE_DAC`, open a second no-reparse security handle with `READ_CONTROL | WRITE_DAC | FILE_READ_ATTRIBUTES`, compare its volume/file identity with the CRT descriptor and current path, apply and verify the protected DACL using `SetSecurityInfo`, then close it. Reject handle/path identity changes without mutating a replacement target, and fail closed if the security API is unavailable. Preserve standalone hook imports and the internal deadline.
