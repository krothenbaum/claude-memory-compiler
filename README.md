...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. A SQLite reservation identifies that exact uncompiled content and starts one detached coordinator only when no interactive Claude Code or Codex session remains. The coordinator rechecks the reservation, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. If another idle drain appends content while compilation runs, the reservation retains the latest pending fingerprint. After the child exits, the coordinator reads the current compiled marker while holding the queue transaction: it releases the reservation when all observed content is covered, cancels when queue work has arrived, or serially launches one coalesced follow-up generation. A failed generation hands off only when the fingerprint advanced; an unchanged failure releases the reservation for a later retry instead of looping. The coordinator renews its lease while compilation runs; an abandoned reservation expires and a later drain replaces it with the latest fingerprint. The automatic compile child also holds a process lock, which prevents overlap if its coordinator crashes. Failed extraction, queued or retrying work, an existing compiled-through marker with no later content, and unavailable process inspection all suppress the automatic compile. The manual `compile.py` command remains available.
