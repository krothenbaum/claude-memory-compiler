...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. SQLite atomically assigns that request either an owner or the single live takeover watcher; only the owner starts compilation. Every later successful idle drain coalesces its latest fingerprint into the reservation and ensures a watcher exists. The watcher heartbeats while the owner lease is live, exits when the marker covers the content, and atomically becomes the owner after expiry. A crashed watcher can be replaced by a subsequent drain. Spawn rollback clears only the failed role, so it cannot erase a concurrent owner or watcher.

The coordinator rechecks the reservation, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. After the child exits, it reads the compiled marker while holding the queue transaction: it releases when all observed content is covered, cancels when queue work arrived, or serially launches one coalesced follow-up generation. Exit code 75 means another automatic child still owns `scripts/memory-auto-compile.lock`; it is deferred, not failed. The owner renews the durable request and polls the lock and marker until the orphan finishes or a retry can launch. Other unchanged failures release for a later retry instead of looping. Failed extraction, queued or retrying work, an existing compiled-through marker with no later content, and unavailable process inspection all suppress automatic compilation. The manual `compile.py` command remains available.
