...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. The first SQLite scheduling transaction atomically provisions both an owner and exactly one takeover watchdog; the watchdog is started first, and only the owner starts compilation. If watchdog startup fails after three bounded local attempts, the matching unstarted pair is rolled back for a later drain without erasing a concurrently recorded generation. If owner startup fails, its lease expires immediately while the live watchdog remains available to take over. Every later successful idle drain coalesces its latest fingerprint into the reservation and replaces an expired watchdog without creating duplicates. During `queue_wait`, `retry_wait`, or unreadable-content wait, a later drain may pre-provision one contender before the live watchdog's lease expires; that process remains available to take over if the watchdog dies without another drain.

The coordinator rechecks the reservation, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. Queue work arriving before launch or during a generation handoff expires the owner lease but preserves the reservation and watchdog; the watchdog waits until pending, leased, and retrying jobs clear, then resumes without another successful drain. Daily reads explicitly distinguish unreadable, covered, and uncompiled states. A child exit of zero succeeds only when the readable post-state is covered or its marker sequence strictly extends the pre-launch sequence without replacement or truncation. Unreadable state, invalid marker history, zero without progress, and other unchanged failures remain durable and retry after bounded exponential backoff; three failed attempts leave an inspectable `failed` request. New content with an unchanged marker starts a fresh generation. On takeover, the predecessor spawns a successor before attempting the SQLite commit and passes both identities to it. The successor accepts a committed watcher identity, or waits out the predecessor watcher/contender lease and atomically registers itself after rollback or predecessor death; another live predecessor keeps renewing the lease, so it cannot be displaced. Exit code 75 remains deferred while the owner polls the lock and marker. Failed extraction, queued or retrying work, fully compiled content, and unavailable process inspection suppress a new request. Process inspection uses bounded argv-list commands: `ps` on POSIX and a filtered, non-interactive PowerShell CIM query on Windows. Only exact bundled Claude argv paths and the complete memory-provider Codex command shape are excluded. The manual `compile.py` command remains available.
