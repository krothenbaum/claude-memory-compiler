...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. The first SQLite scheduling transaction atomically provisions both an owner and exactly one takeover watchdog; the watchdog is started first, and only the owner starts compilation. If watchdog startup fails after three bounded local attempts, the matching unstarted pair is rolled back for a later drain without erasing a concurrently recorded generation. If owner startup fails, its lease expires immediately while the live watchdog remains available to take over. Every later successful idle drain coalesces its latest fingerprint into the reservation and replaces an expired watchdog without creating duplicates. On takeover, the watchdog transaction starts and registers a fresh successor watchdog before the current process becomes owner, so repeated owner crashes remain recoverable without another session drain.

The coordinator rechecks the reservation, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. Queue work arriving before launch or during a generation handoff expires the owner lease but preserves the reservation and watchdog; the watchdog waits until pending, leased, and retrying jobs clear, then resumes without another successful drain. A child exit of zero is successful only when the daily log's `@compiled-through` marker advances, including a marker-only compile that creates no article. Zero without marker progress and other unchanged failures remain durable and retry after bounded exponential backoff; three failed attempts leave an inspectable `failed` request, and a later successful idle drain may retry it. New uncompiled content resets the generation's retry count. Exit code 75 means another automatic child still owns `scripts/memory-auto-compile.lock`; it is deferred while the owner polls the lock and marker until the orphan finishes or a retry can launch. Failed extraction, queued or retrying work, an existing compiled-through marker with no later content, and unavailable process inspection all suppress a new automatic request. Process inspection uses bounded argv-list commands: `ps` on POSIX and a filtered, non-interactive PowerShell CIM query on Windows. Only exact bundled Claude argv paths and the complete memory-provider Codex command shape are excluded. The manual `compile.py` command remains available.
