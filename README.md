...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. The first SQLite scheduling transaction atomically provisions both an owner and exactly one takeover watchdog; the watchdog is started first, and only the owner starts compilation. If watchdog startup fails after three bounded local attempts, the matching unstarted pair is rolled back for a later drain. If owner startup fails, its lease expires immediately while the live watchdog remains available to take over. Every later successful idle drain coalesces its latest fingerprint into the reservation and replaces an expired watchdog without creating duplicates. The watchdog heartbeats while the owner lease is live, exits when the marker covers the content, and atomically becomes the owner after expiry.

The coordinator rechecks the reservation, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. After the child exits, it reads the compiled marker while holding the queue transaction: it releases when all observed content is covered, cancels when queue work arrived, or serially launches one coalesced follow-up generation. Exit code 75 means another automatic child still owns `scripts/memory-auto-compile.lock`; it is deferred, not failed. The owner renews the durable request and polls the lock and marker until the orphan finishes or a retry can launch. Other unchanged failures remain durable and retry after bounded exponential backoff; three failed attempts leave an inspectable `failed` request, and a later successful idle drain may retry it. New uncompiled content resets the generation's retry count. Failed extraction, queued or retrying work, an existing compiled-through marker with no later content, and unavailable process inspection all suppress automatic compilation. Process inspection uses bounded argv-list commands: `ps` on POSIX and a filtered, non-interactive PowerShell CIM query on Windows. Only exact bundled Claude argv paths and the complete memory-provider Codex command shape are excluded. The manual `compile.py` command remains available.
