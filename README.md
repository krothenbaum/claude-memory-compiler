...
After 4 PM local time, a live worker that appended at least one daily entry waits for the queue to become idle, then checks for uncompiled content in today's log. The first SQLite scheduling transaction records the current marker sequence, provisions an owner and one takeover watchdog, and spawns both before commit. The watchdog carries every scheduling input. It joins the committed role or, after a failed owner spawn or transaction, bootstraps recovery only when the queue remains idle and the daily log still matches those inputs. It waits longer than the bounded scheduling transaction, so a slow commit cannot cause false recovery. Replacement watchdogs and pre-provisioned `queue_wait`, `retry_wait`, or unreadable-content contenders use the same spawn-before-commit protocol, closing the database-to-process crash gap while retaining one contender per predecessor.

The coordinator rechecks the reservation, required marker prefix, content fingerprint, and queue idleness in the same transaction that launches `compile.py`. Queue work arriving before launch or during a generation handoff expires the owner lease but preserves the reservation and watchdog; the watchdog waits until pending, leased, and retrying jobs clear, then resumes without another successful drain. It also waits while the owner lease or compiler lock remains live, so a staged marker cannot be mistaken for committed progress. Daily reads explicitly distinguish unreadable, covered, and uncompiled states. A child exit of zero succeeds only when the readable post-state's marker sequence strictly extends the durable required prefix without replacement or truncation; coverage without marker progress is a durable failure. The generation handoff rereads the daily file inside its immediate transaction so a later same-log append is promoted. Unreadable state, invalid marker history, zero without progress, and other unchanged failures preserve the required prefix and retry after bounded exponential backoff; three failed attempts leave an inspectable `failed` request. New uncompiled content with an unchanged marker starts a fresh generation, but cannot erase the required history. On takeover, the predecessor spawns a successor before attempting the SQLite commit and passes both identities to it. The successor accepts a committed watcher identity, or waits out the predecessor watcher/contender lease and atomically registers itself after rollback or predecessor death; another live predecessor keeps renewing the lease, so it cannot be displaced. Exit code 75 remains deferred while the owner polls the lock and marker. Failed extraction, queued or retrying work, fully compiled content, and unavailable process inspection suppress a new request. Process inspection uses bounded argv-list commands: `ps` on POSIX and a filtered, non-interactive PowerShell CIM query on Windows. Only exact bundled Claude argv paths and the complete memory-provider Codex command shape are excluded. The manual `compile.py` command remains available.
...
Run `uv sync` in the repository before merging or updating the Codex hooks. The
Codex commands use `uv run --no-sync` so dependency resolution cannot consume
...
Status records contain operational metadata: source agent, project and session identity, phase, provider, timing, bounded result summaries, and redacted errors. They exclude transcript bodies, prompts, provider output, and extracted knowledge. Recent recognized hook errors from `scripts/logs/hooks.log` appear as health alerts. The structured hook log and other diagnostic logs remain the fallback when a failure occurs before SQLite can accept a status record.
